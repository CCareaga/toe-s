...
// stucture of the 8 byte idt entry
// low:     the lower 16 bits of the interrupt routine addr
// sel:     the segment selector for this entry
// zero:    unused, always set to 0
// flags:   type and attr of the idt entry
// high:    you guessed it, upper 16 bits of int routin addr

typedef struct idt_e {
    uint16_t low;
    uint16_t  sel;
    uint8_t  zero;
    uint8_t  flags;
...
// create an array of 256 entries, not all will be used
idt_e entries[256] = {0};

// create an desc. table pointer to give to the cpu
dt_p idt_ptr;
...
static void idt_add_entry(uint8_t vec, uintptr_t add, uint16_t sel, uint8_t flags) {
...
void idt_init() {
    // set up the idt ptr the same as the gdt, base addr and size
    idt_ptr.lim  = sizeof(idt_e) * 256 - 1;
    idt_ptr.base = (uint32_t) &entries;

    pic_remap();
    
    // add all the entries, i could have probably used a for loop
    // and looked up the addresses in a symbol file but whatever
    ...
    // call the asm function to install the newly created idt
